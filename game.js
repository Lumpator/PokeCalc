...
function showScreen(name) {
  ...
  switch (name) {
    ...
    case 'battle': Battle.start(); break;
...
function renderMenu() {
  ...
  const nextGenThreshold = !gens.includes(2) ? 50 :
                           !gens.includes(3) ? 100 :
                           !gens.includes(4) ? 150 : null;
...
const Battle = {
  ...
  nextQuestion() {
    ...
    answersEl.innerHTML = answers.map(ans =>
  ...
  showHitEffect(imgId) {
    ...
    if (img) {
      ...
      setTimeout(() => img.classList.remove('shake-anim', 'damage-flash'), 400);
    }
  },
  ...
  updateHpBars() {
    ...
    if (playerHp) {
      ...
      playerHp.className = 'hp-bar-fill ' + getHpColorClass(this.playerHp);
    }
...
function renderPokedex(gen) {
  ...
  for (let g = 1; g <= 4; g++) {
    ...
    const cls = (g === currentGen ? 'active' : '') + (unlocked ? '' : ' locked');
